...
export default function Piano({ offset = [6, 4, 0], rotation = [0, -Math.PI / 3, 0] }: PianoProps) {
  const keys = [];
  ...
  for (let i = 0; i < 88; i++) {
    ...
    if (isBlackKey) {
      ...
      keys.push(
        <mesh
          key={`black-${i}`}
          position={[blackKeyX, blackKeyHeight / 2 + whiteKeyHeight, -whiteKeyLength / 2 + blackKeyLength / 2]}
          castShadow
          receiveShadow
        >
