...
export default class Track {
  ...
  constructor(index: number, data: Uint8Array<ArrayBuffer>) {
    this.enabled = true;
    this.eventIndex = 0;
    this.pointer = 0;
    this.lastTick = 0;
    ...
    const lastThreeBytes: Uint8Array<ArrayBuffer> = this.data.subarray(
      this.data.length - 3,
      this.data.length,
    );
   ...
   * @return {Track}
   */
  setEventIndexByTick(tick: number): Track {
    tick = tick || 0;

    for (const i in this.events) {
      if (this.events[i].tick >= tick) {
        this.eventIndex = parseInt(i);
        return this;
      }
    }
   ...
   */
  handleEvent(currentTick: number, dryRun?: boolean): MIDIEvent | null {
    dryRun = dryRun || false;

    if (dryRun) {
      const elapsedTicks: number = currentTick - this.lastTick;
      const delta: number = this.getDelta();
      const eventReady: boolean = elapsedTicks >= delta;

      if (this.pointer < this.data.length && (dryRun || eventReady)) {
        const event: MIDIEvent = this.parseEvent();
  ...
  getStringData(eventStartIndex: number): string {
    const varIntLength: number = Utils.getVarIntLength(this.data.subarray(eventStartIndex + 2));
    const varIntValue: number = Utils.readVarInt(
      this.data.subarray(eventStartIndex + 2, eventStartIndex + 2 + varIntLength),
    );
  ...
  parseEvent(): MIDIEvent {
    const eventStartIndex: number = this.pointer + this.getDeltaByteCount();
    const eventJson: MIDIEvent = {
      track: 0,
      delta: 0,
      tick: 0,
      byteIndex: 0,
    ...
    const deltaByteCount: number = this.getDeltaByteCount();
    ...
    if (this.data[eventStartIndex] == 0xff) {
      ...
      switch (this.data[eventStartIndex + 1]) {
        ...
        case 0x51: // Set Tempo
          ...
          eventJson.data = Math.round(
            60000000 /
              Utils.bytesToNumber(this.data.subarray(eventStartIndex + 3, eventStartIndex + 6)),
          );
          this.tempo = eventJson.data as number;
          break;
        ...
        case 0x58: // Time Signature
          ...
          eventJson.timeSignature =
            '' +
            (eventJson.data as Uint8Array<ArrayBuffer>)[0] +
            '/' +
            Math.pow(2, (eventJson.data as Uint8Array<ArrayBuffer>)[1]);
          break;
        case 0x59: // Key Signature
          ...
          if ((eventJson.data as Uint8Array<ArrayBuffer>)[0] >= 0) {
            eventJson.keySignature =
              Constants.CIRCLE_OF_FIFTHS[(eventJson.data as Uint8Array<ArrayBuffer>)[0]];
          } else if ((eventJson.data as Uint8Array<ArrayBuffer>)[0] < 0) {
            eventJson.keySignature =
              Constants.CIRCLE_OF_FOURTHS[Math.abs((eventJson.data as Uint8Array<ArrayBuffer>)[0])];
          }

          if ((eventJson.data as Uint8Array<ArrayBuffer>)[1] == 0) {
            eventJson.keySignature += ' Major';
          } else if ((eventJson.data as Uint8Array<ArrayBuffer>)[1] == 1) {
      ...
      const varIntLength: number = Utils.getVarIntLength(this.data.subarray(eventStartIndex + 2));
      const length: number = Utils.readVarInt(
        this.data.subarray(eventStartIndex + 2, eventStartIndex + 2 + varIntLength),
      );
      ...
      const varQuantityByteLength: number = Utils.getVarIntLength(
        this.data.subarray(eventStartIndex + 1),
      );
      const varQuantityByteValue: number = Utils.readVarInt(
        this.data.subarray(eventStartIndex + 1, eventStartIndex + 1 + varQuantityByteLength),
      );
      ...
      if (this.data[eventStartIndex] < 0x80) {
        ...
        if (this.lastStatus !== null && this.lastStatus <= 0x8f) {
          eventJson.name = 'Note off';
          eventJson.channel = this.lastStatus - 0x80 + 1;
          this.pointer += deltaByteCount + 2;
        } else if (this.lastStatus !== null && this.lastStatus <= 0x9f) {
          eventJson.name = 'Note on';
          eventJson.channel = this.lastStatus - 0x90 + 1;
          this.pointer += deltaByteCount + 2;
        } else if (this.lastStatus !== null && this.lastStatus <= 0xaf) {
          ...
          this.pointer += deltaByteCount + 2;
        } else if (this.lastStatus !== null && this.lastStatus <= 0xbf) {
          // Controller Change
